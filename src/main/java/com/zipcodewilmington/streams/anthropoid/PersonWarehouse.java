...
public final class PersonWarehouse implements Iterable<Person> {
    ...
    public Stream<Person> getUniquelyNamedPeopleStartingWith(Character character) {
        return people.stream().filter(e -> e.getName().charAt(0) == character).distinct();
    }
    ...
    public Stream<Stream<String>> getNestedAliases() {
        return people.stream().map(e -> Arrays.stream(e.getAliases()));
    }
    ...
    public Stream<String> getAllAliases() {
        return people.stream().map(e -> Arrays.toString(e.getAliases()));
    }
